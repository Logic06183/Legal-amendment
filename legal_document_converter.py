"""
Specialized Legal Document Converter
Creates professionally formatted Word documents with proper legal formatting and tracked changes.
Designed to produce documents that lawyers can easily review and edit.
"""
...
def apply_legal_document_styles(doc):
    """Apply professional legal document styling"""
...
def convert_markdown_to_word(markdown_file, output_docx, include_legend=True):
    ...
    doc = Document()
    doc = apply_legal_document_styles(doc)
    
    # Add title with proper legal document formatting
    ...
    in_paragraph = False
    current_text = ""
    skip_lines = 7 if include_legend else 1  # Skip title and legend lines
...
def main():
    ...
    output_dir = os.path.join(base_dir, "Legal_Review_Documents")
    ...
    for input_dir in input_dirs:
        ...
        for file_name in os.listdir(input_dir):
            if file_name.endswith("_With_Changes.md"):
                ...
                output_file = os.path.join(output_subdir, f"{base_name}_Legal.docx")
                
                # Convert the file
                convert_markdown_to_word(markdown_file, output_file)
    
    print("\nConversion complete! Legal review documents have been created in:")
    print(output_dir)
    ...
    print("5. Standardized heading styles")
    print("\nLawyers can now review and edit these documents directly in Word.")
